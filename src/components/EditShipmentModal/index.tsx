...
export function EditShipmentModal({ isOpen, onRequestClose, setCarregamentos, id }: NewShipmentModal) {
  ...
  useEffect(() => {
    ...
    api.get(`carregamento/${id}`).then(res => {
      const data = res.data[0] ? res.data[0].expedicao : ""
      setExpedicao(data)
    })

    api.get(`itens/${id}`).then(res => {
      res.data.forEach((dados: dadosItens, index: number) => {
        const data = new Date(dados.createdat)
        const dataFormat = `${data.getFullYear()}-${data.getMonth() > 10 ? data.getMonth() : '0' + data.getMonth()}-${data.getDate() > 10 ? data.getDate() : '0' + data.getDate()}`

        setSelections((prevState) => [...prevState, String(dados.tipo_orcamento)])
        setOrdemProducao((prevState) => [...prevState, String(dados.ordem_producao)])
        setNomeCliente((prevState) => [...prevState, dados.nome_cliente])
        setDataConclusao((prevState) => [...prevState, dataFormat])
        setRomaneio((prevState) => [...prevState, String(dados.romaneio)])
        setRelatorio((prevState) => [...prevState, String(dados.relatorio)])
        
        setProduto((prevState) => [...prevState, dados.produto])
      })
    })
  }, [id]) 
  ...
  const saveButton = async (ev: { preventDefault: () => void; }) => {
    ...
    const response = await api.put(`/carregamento/${id}`, {
      expedicao,
    ...
    setCarregamentos((prevState) => prevState.filter((dados) => dados.id !== id))
    setCarregamentos((prevState) => [carregamento, ...prevState])
    
    const id_carregamento = response.data[0].id

    await api.delete(`/itens/${id_carregamento}`)
    ...
    for(let i = 0; i < selections.length; i++) {
      await api.post("/itens", {
        ...
        createdAt: new Date(dataReal),
        updatedAt: new Date(dataReal),
        id_carregamento: Number(id_carregamento),
